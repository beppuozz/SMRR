...
G4VPhysicalVolume* DetectorConstruction::ConstructHadCalo()
{
	...
	HadCaloSensitiveDetector* HadCalo = 0;
	if(!HadCalo) {
		G4cout << "Creating had calo sensitive detector " << G4endl;
		HadCalo = new HadCaloSensitiveDetector("HadCalo");
		G4SDManager::GetSDMpointer()->AddNewDetector(HadCalo);
		G4cout << "Sensitive detector added " << G4endl;
	}
	hadCaloLogic->SetSensitiveDetector(HadCalo);
	...
	//Create sensitive detector for lAr layer
	hadLayerLogic->SetSensitiveDetector(HadCalo);
